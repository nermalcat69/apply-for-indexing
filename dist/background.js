...
function extractLocs(xml) {
  ...
  while ((match = locRegex.exec(xml)) !== null) {
    const url = match[1].trim().replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'");
    if (url) results.push(url);
  }
  return results;
}
async function parseSitemapXml(text) {
  ...
  (message, _sender, sendResponse) => {
    (async () => {
      switch (message.type) {
        ...
        case "STOP_INDEXING": {
          const state = await getState();
          const urls = state.urls.map(
            (u) => u.status === "processing" ? { ...u, status: "pending" } : u
          );
        ...
        case "URL_RESULT": {
          const { url, status, message: msg } = message.payload;
          const state = await getState();
          const urls = state.urls.map(
            (entry) => entry.url === url ? { ...entry, status, message: msg, processedAt: Date.now() } : entry
          );
